...
URL を入力するだけで、サイトの HTML/CSS/画像/フォントを丸ごとダウンロードし、ヘッダー・ヒーロー・料金表・FAQ・フッターといったセクション単位に自動分割。Claude Code CLI（サブスク）で 18 種類のカテゴリに AI 分類し、ライブラリに蓄積。**要素単位のドラッグ&ドロップ**で異なるサイトのパーツを自由に組み合わせ、統一されたデザインのページを構築できます。
...
| **要素単位 D&D** | セクション内の個別要素（ボタン・見出し・画像等）をドラッグして別セクションに配置。サイト A のボタンをサイト B のヘッダーに移植可能 |
| **ビジュアル編集** | パーツ内のテキスト・画像・リンクをクリックして直接編集（WordPress 風インライン編集） |
| **スタイル編集** | 選択要素の色・フォント・余白・角丸をビジュアルエディタで直接変更。カラーピッカー対応 |
| **テーマ一括適用** | メインカラー・背景色・フォントを設定し、全セクションに統一テーマを一括適用 |
| **要素の複製・削除** | 選択要素のワンクリック複製・削除。Undo/Redo 対応（Ctrl+Z / Ctrl+Shift+Z） |
| **HTML コード編集** | Monaco Editor（VS Code 相当）でパーツの HTML を直接書き換え。ライブプレビュー付き |
| **AI デザイン統一** | 複数サイトから集めたバラバラなデザインを Claude Code CLI で統一。ブランドカラー・業種・ターゲット設定可。API キー不要 |
| **画像 URL 表示** | セクション内の画像元 URL を一覧表示。クリックでコピー・新タブ表示 |
| **プロジェクト管理** | Canvas の状態を保存・復元。複数プロジェクト対応 |
| **Canvas 自動バックアップ** | Canvas 状態をサーバーに自動保存。ブラウザ変更やキャッシュクリアでもデータ復元可能 |
| **HTML エクスポート** | 完成ページを単体 HTML ファイルとしてダウンロード。CSS スコーピングで複数サイトのスタイルが競合しない |
...
AI 分類・デザイン統一は **Claude Code CLI**（サブスク）を使用。Anthropic API キーは不要。

| 処理 | 実行方法 | 追加コスト |
|------|---------|-----------|
| **セクション分類** | Claude Code CLI (`claude --print`) | **$0（サブスク内）** |
| **デザイン統一**（任意） | Claude Code CLI (`claude --print`) | **$0（サブスク内）** |

> Claude Code 未インストール時はヒューリスティック分類にフォールバック。
...
| 構成 | 月額目安 |
|------|---------|
| **最小構成**（ローカル + Claude Code サブスク） | **Claude Code サブスク料のみ** |
| **フル構成**（Supabase Pro + Claude Code） | **$25/月 + Claude Code サブスク** |
...
Claude Code CLI（サブスク）経由で 18 種の Block Family に分類（15 セクション/バッチ）。信頼度スコア（0〜1）と品質スコア（0〜1）を付与。CLI 未インストール時はルールベースのヒューリスティック分類にフォールバック。API キー不要・追加コストなし。
...
│   ├── claude-cli.ts          # Claude Code CLI ラッパー（サブスク利用）
│   ├── ai-classifier.ts      # Claude CLI AI 分類（バッチ処理）
